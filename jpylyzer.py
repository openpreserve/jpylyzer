...
def generatePropertiesRemapTable():
    ...
    # Bits per component:
    #  sign (Image HeaderBox, Bits Per Component Box, SIZ header
    # in codestream)
    signMap = {}
    signMap[0] = "unsigned"
    signMap[1] = "signed"
...
def checkOneFile(file):
    # Process one file and return analysis result as
    # text string (which contains formatted XML)

    fileData = readFileBytes(file)
    isValidJP2, tests, characteristics = \
            BoxValidator("JP2", fileData).validate()
    ...
    # File name and path may contain non-ASCII characters,
    # decoding to Latin should (hopefully) prevent any
    # Unicode decode errors. Elementtree will then deal with any non-ASCII
    # characters by replacing them with numeric entity references
    try:
        # This works in Python 2.7, but raises
        # error in 3.x (no decode attribute for str type!)
...
def main():
    ...
    # Storing this to 'config.outputVerboseFlag' makes
